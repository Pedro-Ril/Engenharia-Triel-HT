...
async function handlePOST(request: Request, context: RouteContext) {
  ...
  try {
     ...
     * não na digitação/blur (ver rota .../nf-saida). Não bloqueia o
     * empréstimo se essa consulta falhar: tentarBuscarNfSaida nunca
    ...
    if (equipamentoAntes?.codigoEmpresa) {
      await tentarBuscarNfSaida({
        ...
        tipoNf: "saida_emprestimo",
        disparadoPor: usuario.nomeExibicao,
      });
    }

    const equipamento = await registrarEmprestimo({
      equipamentoId: id,
      numeroNf,
      destinatarioNome,
      valor,
    ...
    return NextResponse.json({ ok: true, message: "Empréstimo registrado.", data: equipamento });
  } catch (error) {
    if (error instanceof ValidationError) {
      return NextResponse.json({ ok: false, message: error.message }, { status: 400 });
    }

    console.error("Erro ao registrar empréstimo de equipamento:", error);
    return NextResponse.json(
      { ok: false, message: "Não foi possível registrar o empréstimo." },
      { status: 500 }
    );
  }
}

export const POST = comMetricasApi("estoque-equipamentos-usados/[id]/emprestimo", handlePOST);
