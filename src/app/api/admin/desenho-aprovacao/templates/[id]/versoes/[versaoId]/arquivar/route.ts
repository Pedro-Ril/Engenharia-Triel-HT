...
import { arquivarVersao } from "@/lib/desenho-aprovacao/templates";
...
async function handlePOST(_request: Request, context: RouteContext) {
  ...
  try {
    const versao = await arquivarVersao(versaoId, acesso.usuario.samAccountName);
    return NextResponse.json({ ok: true, message: "Versão arquivada.", data: versao });
  } catch (error) {
    if (error instanceof ValidationError) {
      return NextResponse.json({ ok: false, message: error.message }, { status: 409 });
    }

    console.error("Erro ao arquivar versão de template:", error);

    return NextResponse.json({ ok: false, message: "Não foi possível arquivar a versão." }, { status: 500 });
  }
}

export const POST = comMetricasApi(
  "admin/desenho-aprovacao/templates/[id]/versoes/[versaoId]/arquivar",
  handlePOST
);
