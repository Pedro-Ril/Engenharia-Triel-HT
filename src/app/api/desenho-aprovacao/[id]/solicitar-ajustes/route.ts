...
async function handlePOST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          message:
            `O desenho está com o status "${current.statusDesenho}" e não pode receber uma solicitação de ajustes.`,
        },
        {
          status: 409,
        }
      ...
      return NextResponse.json(
        {
          ok: false,
          message:
            `A revisão ${current.codigoRevisao} está com o status "${current.statusRevisao}".`,
        },
        {
          status: 409,
        }
    ...
    const result =
      await databaseRequest.query<AdjustmentResult>(`
        SET XACT_ABORT ON;
        ...
        BEGIN
          THROW 50001,
            'A revisão foi alterada durante a solicitação de ajustes.',
            1;
        END;
        ...
        BEGIN
          THROW 50002,
            'O desenho foi alterado durante a solicitação de ajustes.',
            1;
        END;
    ...
    return NextResponse.json({
      ok: true,
      message:
        `Ajustes solicitados para a revisão ${current.codigoRevisao}.`,
    ...
    return NextResponse.json(
      {
        ok: false,
        message:
          "Não foi possível solicitar ajustes para a revisão.",
      },
      {
        status: 500,
      }
...
export const POST = comMetricasApi("desenho-aprovacao/[id]/solicitar-ajustes", handlePOST);